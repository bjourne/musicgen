...
Usage:
    generate-midi.py [-hvo MIDI] [--programs=<seq>] module
        [--midi-mapping=<json> --use-mcode] <mod>
...
Options:
    ...
    --use-mcode            use the mcode intermediate format
...
def mod_file_to_midi_file_using_mcode(mod_file, midi_file):
    # Can't use packing here since it is not exact.
    mcode = mod_file_to_mcode(mod_file, False)
    time_ms = mcode.time_ms
    SP.print('Rowtime %d ms.' % time_ms)
    notes = [mcode_to_mod_notes(seq, i, time_ms, pitch_idx, None)
             for i, (pitch_idx, seq)
             in enumerate(mcode.cols)]
    notes = flatten(notes)
    notes_to_midi_file(notes, midi_file, MCODE_MIDI_MAPPING)

def random_cache_location(mcode_mods, n_insns):
    long_jump_tok = INSN_JUMP, 64
    while True:
        i = randrange(len(mcode_mods))
        mcode_mod = mcode_mods[i]
        j = randrange(4)
        _, col = mcode_mod.cols[j]
        n = len(col)
        if n - n_insns <= 0:
...
def cache_file_to_midi_file(cache_file, midi_file,
                            ...
                            programs,
                            guess):
    mcode_mods = load_pickle(cache_file)
    if loc == 'random':
        mod_idx, col_idx, seq_idx = random_cache_location(mcode_mods,
                                                          n_insns)
    else:
        mod_idx, col_idx, seq_idx = [int(i) for i in loc.split(':')]
        col_idx -= 1

    mcode_mod = mcode_mods[mod_idx]
    pitch_idx, seq = mcode_mod.cols[col_idx]
    seq = seq[seq_idx:seq_idx + n_insns]

    fmt = '%d instructions from "%s" %d:%d:%d.'
    SP.print(fmt, (n_insns, mcode_mod.name,
                   mod_idx, col_idx + 1, seq_idx))
    if guess:
        pitch_idx = None
    mcode_to_midi_file(seq, midi_file, mcode_mod.time_ms, pitch_idx)
...
def main():
    ...
    if mod_file:
        ...
        if args['--use-mcode']:
            mod_file_to_midi_file_using_mcode(mod_file, midi_file)
        else:
